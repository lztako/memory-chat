...
export default async function InvoicesPage() {
  ...
  const files = await prisma.userFile.findMany({
    where: { userId: user.id, fileType: "invoice" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: "var(--text3)", margin: "3px 0 0" }}>
            {files.length > 0 ? `${totalRows.toLocaleString()} records across ${files.length} file${files.length !== 1 ? "s" : ""}` : "No invoice files uploaded yet"}
          </p>
        </div>
        <Link
          href="/chat"
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 28px" }}>
        {files.length === 0 ? (
          <EmptyState
            label="No invoice data"
            hint="Upload an invoice CSV file to see your data here"
            href="/settings"
          />
        ) : (
